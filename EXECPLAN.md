...
The main remaining technical gap is no longer funded-regtest practicality, block-acceptance proof, canary sync, or the live QSB funding/spend proof. Those were resolved by fixing the regtest no-retargeting regression, adding a two-node mining test where an unmodified peer accepts QSB-carrying blocks, letting the upgraded canary complete IBD, adding explicit funding fee-rate control, and mining the live funding/spend pair on `contabo-validator-01`. The remaining operational work is narrower: `contabo-validator-02`, `contabo-validator-04`, and `contabo-validator-05` already have the validated binary/config shape, but they must finish CPU-bound header and block sync before mining can be safely re-enabled, the temporary `minimumchainwork=0` catch-up override can be removed, and their final tips can be recorded. A 2026-04-10 04:10Z recheck was still a no-go: 02 remained in IBD at block `3167`, 04 remained in IBD around block `5700`, and 05 was active but CPU-bound enough that heavier chain RPCs timed out.
