...
BOOST_AUTO_TEST_CASE(key_test1)
{
    ...
    const auto CheckPubkeyAddress = [](const CPubKey& pubkey) {
        const CTxDestination expected{PKHash(pubkey)};
        const std::string encoded{EncodeDestination(expected)};
        BOOST_CHECK(DecodeDestination(encoded) == expected);
    };

    CheckPubkeyAddress(pubkey1);
    CheckPubkeyAddress(pubkey2);
    CheckPubkeyAddress(pubkey1C);
    CheckPubkeyAddress(pubkey2C);
