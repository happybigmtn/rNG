...
{
    InitializeForTest(their_pubkey, initiator, Params().MessageStart(), self_decrypt);
}

void BIP324Cipher::InitializeForTest(const EllSwiftPubKey& their_pubkey, bool initiator, std::span<const uint8_t> message_start, bool self_decrypt) noexcept
{
    // Determine salt (fixed string + network magic bytes)
    std::string salt{"bitcoin_v2_shared_secret"};
    salt.append(reinterpret_cast<const char*>(message_start.data()), message_start.size());
