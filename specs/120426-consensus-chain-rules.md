...
Define the consensus rules governing RNG's blockchain: block timing, subsidy schedule, difficulty adjustment, coinbase maturity, weight limits, and soft-fork activation baseline. This spec captures the live mainnet behavior as implemented in Bitcoin Core v30.2-derived code with RNG-specific modifications.
...
- **Tail emission floor**: none in live consensus. `GetBlockSubsidy()` halves `50 * COIN` and returns `0` once `halvings >= 64`.
...
- **Current deployments**: `DEPLOYMENT_TESTDUMMY`, `DEPLOYMENT_TAPROOT`, and dormant `DEPLOYMENT_SHAREPOOL` are defined in `src/consensus/params.h`
...
- Sharepool activation uses the existing BIP9 machinery. The live dormant deployment uses bit 3, period 2016, and threshold 1916/2016 on mainnet, testnet, testnet4, and signet; regtest uses period 144 and threshold 108.
...
- Whether the historical 0.6 RNG tail-emission goal should be implemented later. The live code has no tail floor and `MAX_MONEY = 21,000,000 * COIN`.
...
- After the final halving, subsidy eventually reaches zero; there is no live tail-emission floor
...
- `GetBlockSubsidy()` returns zero once `halvings >= 64`
...
2. Should the historical 0.6 RNG tail-emission goal be implemented, or should supply docs keep the current Bitcoin-derived no-tail subsidy schedule?
